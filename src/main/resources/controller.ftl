...
*/
@RestController
@RequestMapping("${entityClass?lower_case}s")
public class ${entityClass}Controller implements CrudController<${entityClass},${primaryKeyClass}>{
    ...
    @Override
    public ResponseEntity<Page<${entityClass}>> get(
            @RequestBody ${entityClass} entity,
            @RequestParam("page") Integer page,
            @RequestParam("size") Integer size) {
        Pageable pageable = PageRequest.of(page,size);
        return ResponseEntity.ok(service.get(entity,pageable));
    }

    @Override
    public ResponseEntity<${entityClass}> get(@PathVariable("id") ${primaryKeyClass} primaryKey) {
        return ResponseEntity.ok(service.get(primaryKey));
    }
