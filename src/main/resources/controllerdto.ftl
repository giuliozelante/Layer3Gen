...
*/
@RestController
@RequestMapping("${entityClass?lower_case}s-dto")
public class ${entityClass}ControllerDTO implements CrudController<${entityClass}DTO,${primaryKeyClass}>{
    ...
    @Override
    public ResponseEntity<Page<${entityClass}DTO>> get(
            @RequestBody ${entityClass}DTO dto,
            @RequestParam("page") Integer page,
            @RequestParam("size") Integer size) {
        Pageable pageable = PageRequest.of(page,size);
        ${entityClass} entity = mapper.toEntity(dto);
        Page<${entityClass}DTO> pages = service.get(entity, pageable).map(mapper::toDto);
        return ResponseEntity.ok(pages);
    }

    @Override
    public ResponseEntity<${entityClass}DTO> get(@PathVariable("id") ${primaryKeyClass} primaryKey) {
         ${entityClass} entity = service.get(primaryKey);
         return ResponseEntity.ok(mapper.toDto(entity));
    }
